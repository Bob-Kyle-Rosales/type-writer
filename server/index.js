      ...
      if (err) {
        ...
        db.all("PRAGMA table_info(documents)", (err, columns) => {
          if (err) {
            ...
            if (!hasUserId) {
              console.log('Migrating documents table to add user_id column...');
              ...
              if (!hasName) {
                console.log('Migrating documents table to add name column...');
                db.run('ALTER TABLE documents ADD COLUMN name TEXT DEFAULT "Untitled Document"', (err) => {
                  if (err) {
                    console.error('Error adding name column:', err.message);
                  } else {
                    console.log('Documents table migrated with name column');
                  }
                });
              } else {
...
app.post('/api/documents', authenticateToken, (req, res) => {
  ...
  db.run('INSERT INTO documents (user_id, name, content) VALUES (?, ?, ?)', 
    [req.userId, documentName, content], function(err) {
    if (err) {
...
app.put('/api/documents/:id', authenticateToken, (req, res) => {
  ...
  if (name !== undefined) {
    db.run('UPDATE documents SET content = ?, name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?', 
      [content, name, id, req.userId], function(err) {
      if (err) {
    ...
    db.run('UPDATE documents SET content = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?', 
      [content, id, req.userId], function(err) {
      if (err) {
        console.error('Error updating document:', err.message);
        return res.status(500).json({ error: 'Failed to update document' });
      }
...
app.get('/api/documents', authenticateToken, (req, res) => {
  db.all('SELECT id, name, content, created_at, updated_at FROM documents WHERE user_id = ? ORDER BY updated_at DESC', 
    [req.userId], (err, rows) => {
    if (err) {
...
app.get('/api/documents/:id', authenticateToken, (req, res) => {
  ...
  db.get('SELECT id, name, content, created_at, updated_at FROM documents WHERE id = ? AND user_id = ?', 
    [id, req.userId], (err, row) => {
    if (err) {
...
app.get('/api/documents/latest', authenticateToken, (req, res) => {
  db.get('SELECT id, name, content, created_at, updated_at FROM documents WHERE user_id = ? ORDER BY updated_at DESC LIMIT 1', 
    [req.userId], (err, row) => {
    if (err) {
    ...
    if (!row) {
      return res.json({ id: null, name: '', content: '', message: 'No documents found' });
    }
...
app.delete('/api/documents/:id', authenticateToken, (req, res) => {
  ...
  db.run('DELETE FROM documents WHERE id = ? AND user_id = ?', 
...
app.delete('/api/documents', authenticateToken, (req, res) => {
  db.run('DELETE FROM documents WHERE user_id = ?', [req.userId], function(err) {
    if (err) {
